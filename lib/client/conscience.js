...
(function() {
	...
	*/
	...
	omni.Conscience = function(name, token) {
		...
		var thoughtNamespace = io(omni.host + omni.getNameForNamespace(this.cleanName, omni.THOUGHT));
		this.thoughtNamespace = thoughtNamespace;

		var stateNamespace = io(omni.host + omni.getNameForNamespace(this.cleanName, omni.STATE));
	...
	omni.Conscience.prototype.requestToken = function() {
		var obj = this;
		this.thoughtNamespace.emit(omni.REQUEST_TOKEN, "", function(token) {
			obj.token = token;
			obj.getState("", function(state) {
				obj.state = state;
				obj.stateHandler.token = token;
				obj.onTokenGranted(token);
			})
		})
	}
	 ...
	 */
	omni.Conscience.prototype.onTokenGranted = function(token) {
		//This function is to be overwritten
	}
	 ...
	 * @param [Array] receivers - an array of tokens indicating the clients of which to receieve the thought.
				Specify an empty array to send to only the server. Specify null or leave undefined to send to all clients.
	 */
