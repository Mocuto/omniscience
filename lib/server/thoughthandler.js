(function() {
	...
	/**
	 *  @class (server) ThoughtHandler
	 *  @classdesc Handles communicating with clients regarding thoughts.
	...
	*/
	...
	omni.ThoughtHandler.prototype.send = function(thought, id) {
		...
		else if (id !== omni.SERVER)
		{
			//TODO: Create onRecipientDoesntExist callback
		}
	};
	 ...
	 * @public
	 * @memberof (server) omni.(server) ThoughtHandler#
	 * @desc Removes the specified socket from the list of receivers.
	 * @param {Any} id - The unique identifier of the client to remove.
	 */
	omni.ThoughtHandler.prototype.removeSocket = function(id) {
	...
	omni.ThoughtHandler.prototype.applyFunctionToSocket = function(name, callback, callee, socket) {
		...
		socket.on(name, function(thought, acknowledgementFunction) { 
			if (typeof thought.sender !== "undefined" && thought.sender != null)
			{
				...
				if (typeof acknowledgementFunction === "function")
				{
